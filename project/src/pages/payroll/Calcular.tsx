...
const mockPeriodo = (id: string, sucursal_id = 'demo-1'): Periodo => {
  ...
  return {
    id, sucursal_id, periodo_mes: mes, periodo_ano: ano,
    fecha_inicio: inicio.toISOString(), fecha_fin: fin.toISOString(),
    estado: 'borrador', created_at: now.toISOString(),
  };
};
...
export default function Calcular() {
  ...
  /* ✅ Solo UNA definición de showSyncButton */
  ...
  const totals = useMemo(
    () => ({ salario: sum(detalle, 'salario_base'), horas: sum(detalle, 'horas'), total: sum(detalle, 'total') }),
    [detalle]
  );

  useEffect(() => { window.__DEMO_MODE__ = isDemo; }, [isDemo]);

  useEffect(() => {
    if (typeof window === 'undefined') return;
    if (!sucursalSeleccionada && sucursales.length > 0) {
      const storedId = localStorage.getItem('selectedSucursalId');
      const fallback = storedId ? sucursales.find((s) => String(s.id) === String(storedId)) : undefined;
      if (fallback) setSucursalSeleccionada(fallback);
    }
  }, [sucursalSeleccionada, sucursales, setSucursalSeleccionada]);

  const activateDemo = useCallback((reason?: string) => {
    if (reason) console.warn('[Calcular] Activando datos demo:', reason);
  ...
  const checkResolvedViewAvailability = useCallback(async (periodoIdToCheck: string) => {
    ...
    if (!supabaseUrl || !supabaseAnonKey || typeof fetch === 'undefined') {
      setResolvedViewAvailable(false); setResolvedViewStatus(null); return false;
    }
    try {
      const endpoint =
        `${supabaseUrl}/rest/v1/v_ui_periodo_detalle_resuelto?periodo_id=eq.${encodeURIComponent(periodoIdToCheck)}&select=periodo_id&limit=1`;
      const response = await fetch(endpoint, { headers: { apikey: supabaseAnonKey, Authorization: `Bearer ${supabaseAnonKey}` } });
      setResolvedViewStatus(response.status);
      if (response.ok) { setResolvedViewAvailable(true); return true; }
      setResolvedViewAvailable(false); return false;
    } catch (error) {
  ...
  const loadPeriodo = useCallback(async () => {
    try {
      if (!periodoId) {
        ...
        setLoading(false); setIsDemo(false); setDemoReason(null);
        setDetalle([]); setResumen(null);
        return;
      }
      setLoading(true); setError(''); setIsDemo(false); setDemoReason(null);
      setDetalle([]); setResumen(null); setDetalleSource('hr_periodo_detalle');

      if (demoMode) { activateDemo('Supabase no configurado (shouldUseDemoMode=TRUE)'); setLoading(false); return; }

      const { data: pData, error: pErr } = await supabase.from('hr_periodo').select('*').eq('id', periodoId).single();
      if (pErr) throw pErr;
      if (!pData) throw new Error('No se encontró el período');
      setPeriodo(pData as Periodo);

      const { data: baseData, error: baseErr } = await supabase
        .from('hr_periodo_detalle').select(DETAIL_COLUMNS).eq('periodo_id', periodoId)
      ...
      let canUseResolvedView = resolvedViewAvailable === true || await checkResolvedViewAvailability(periodoId);
      if (canUseResolvedView) {
        const { data: viewData, error: viewErr } = await supabase
          .from('v_ui_periodo_detalle_resuelto').select(DETAIL_COLUMNS).eq('periodo_id', periodoId);
        if (viewErr && viewErr.code !== '42P01') {
          console.warn('[Calcular] Error al usar v_ui_periodo_detalle_resuelto', viewErr);
          setResolvedViewAvailable(false);
        } else if (viewData) {
          finalRows = viewData as PeriodoDetalle[]; finalSource = 'v_ui_periodo_detalle_resuelto';
        }
      }

      setDetalle(finalRows); setDetalleSource(finalSource);

      const { data: resumenData, error: resErr } = await supabase
        .from('v_ui_resumen_planilla').select('*').eq('periodo_id', periodoId).maybeSingle();
      if (resErr && resErr.code !== '42P01') throw resErr;

      if (resumenData) setResumen(resumenData as PeriodoResumen);
      else if (finalRows.length > 0) setResumen(buildMockResumen(finalRows));
  ...
  }, [activateDemo, checkResolvedViewAvailability, demoMode, periodoId, resolvedViewAvailable]);
  ...
  const handleCalcular = useCallback(async () => {
    ...
    try {
      const { error: rpcErr } = await supabase.rpc('rpc_hr_calcular_periodo', { p_periodo_id: periodo.id });
      if (!rpcErr) { await loadPeriodo(); return; }

      const { data: empleados } = await supabase
        .from('hr_empleado').select('id, nombre, salario_base, sucursal_id').limit(200);
  ...
  function handleChangeSucursal(e: React.ChangeEvent<HTMLSelectElement>) {
    ...
    if (nueva) { setSucursalSeleccionada(nueva); localStorage.setItem('selectedSucursalId', nueva.id); }
    navigate('/payroll');
  }

  const BackBar: React.FC = () => {
    const goBack = () => { if (window.history.length > 1) window.history.back(); else navigate('/payroll'); };
    return (
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2">
          <button onClick={goBack} className="inline-flex items-center gap-2 rounded-xl border bg-white px-4 py-2 shadow hover:bg-gray-50" title="Volver">
            <ArrowLeft className="h-4 w-4" /><span>Volver</span>
          </button>
          <a href="/payroll" className="text-sm text-slate-600 underline underline-offset-4 transition hover:text-slate-900" title="Ver períodos">
            Ir a períodos
          </a>
        </div>
        <div className="flex items-center gap-3">
          ...
          {sucursales.length > 0 && (
            <select value={sucursalSeleccionada?.id ?? ''} onChange={handleChangeSucursal} className="rounded-lg border bg-white px-3 py-2 text-sm" title="Cambiar sucursal">
              {sucursales.map((s) => (<option key={String(s.id)} value={String(s.id)}>{s.nombre}</option>))}
            </select>
          )}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border-l-4 border-red-500 bg-red-50 p-4">
          <div className="flex items-center gap-2 text-red-700">
            <AlertCircle className="h-5 w-5" /><span className="font-medium">{error}</span>
          </div>
        </div>
      ) : !periodo ? (
        <div className="rounded-2xl bg-white p-8 text-center shadow">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border bg-white p-6 shadow">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                <h2 className="text-2xl font-semibold">{MESES[periodo.periodo_mes - 1]} {periodo.periodo_ano}</h2>
                <p className="text-gray-600">{formatDateDDMMYYYY(periodo.fecha_inicio)} — {formatDateDDMMYYYY(periodo.fecha_fin)}</p>
                <div className="mt-2">
              ...
              <div className="flex items-center gap-3">
                <button onClick={handleCalcular} disabled={calculando} className="inline-flex items-center gap-2 px-5 py-3 rounded-xl text-white bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 shadow disabled:opacity-50">
                  <PlayCircle className={`h-5 w-5 ${calculando ? 'animate-spin' : ''}`} />
                  {calculando ? 'Calculando…' : 'Calcular planilla'}
                </button>

                <button onClick={loadPeriodo} className="inline-flex items-center gap-2 px-5 py-3 rounded-xl border hover:bg-gray-50 shadow">
                ...
                {showSyncButton && (
                  ...
                  >
                    <Loader2 className="h-4 w-4" />Sincronizar empleados
                  </button>
                )}
              </div>
            </div>
          ...
          <div className="rounded-2xl border bg-white p-6 shadow">
            ...
            {detalle.length === 0 ? (
              <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 py-12 text-center">
                ...
                >
                  <PlayCircle className="h-4 w-4" />Calcular planilla
                </button>
              </div>
            ) : (
                  ...
                  <table className="min-w-full table-fixed text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {detalle.map((row, index) => (
                        <tr key={`${row.empleado_id ?? index}-${index}`} className={index % 2 === 1 ? 'bg-slate-50/60' : 'bg-white'}>
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="min-w-0">
                                <div className="truncate text-sm font-semibold text-slate-900">{row.empleado_nombre ?? 'Empleado sin nombre'}</div>
          ...
          {resumen && (
            <div className="rounded-2xl border bg-white p-6 shadow">
              ...
              <div className="grid grid-cols-2 gap-6 md:grid-cols-4">
                <div><div className="text-sm text-gray-500">Empleados</div><div className="text-lg font-semibold">{resumen.total_empleados ?? '—'}</div></div>
                <div><div className="text-sm text-gray-500">Salarios</div><div className="text-lg font-semibold">{money(resumen.total_salarios)}</div></div>
                <div><div className="text-sm text-gray-500">Deducciones</div><div className="text-lg font-semibold">{money(resumen.total_deducciones)}</div></div>
                <div><div className="text-sm text-gray-500">Total Neto</div><div className="text-lg font-semibold">{money(resumen.total_neto)}</div></div>
              </div>
            </div>
          )}
        </div>
