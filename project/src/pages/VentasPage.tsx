...
export function VentasPage() {
  ...
  const handleSync = useCallback(async () => {
    setSyncing(true);
    try {
      const url = `${functionsBase}/sync-ventas?desde=${desde}&hasta=${hasta}`;
      console.log('[VentasPage] Calling sync endpoint:', url);
      ...
      if (!resp.ok) {
        ...
        console.error('[VentasPage] Sync error:', resp.status, txt);
        throw new Error(`sync-ventas ${resp.status} ${txt}`);
      } else {
        try {
          const js = await resp.json();
          console.log('[VentasPage] Sync response:', js);
          ...
          console.error('[VentasPage] Failed to parse sync response:', e);
        }
      }
      ...
      console.error('Sync Ventas (DB) error:', e);
      setSyncBanner(null);
    } finally {
      setSyncing(false);
    }
  }, [desde, hasta, loadData]);
  ...
  const rt: any = useRealtimeVentas({
    ...
    debounceMs: 1500,
    onUpdate: () => {
      console.log('Realtime update detected in VentasPage, reloading data...');
      loadData();
    },
  });
