...
import {
  ...
  formatFunctionsHost,
  getFunctionsBase,
  getInvuMarcacionesUrlForYesterday,
  isDebug,
  logEnv,
  rest,
  yesterdayUTC5Range,
  ...
  useEffect(() => {
    ...
    if (functionsBase) {
      ...
      setEdgeChecks({
        ...
        marcaciones: { ok: false, status: 0, note: 'Falta VITE_SUPABASE_FUNCTIONS_BASE' },
      });
    }
  ...
  if (!debugMode) {
    return (
      <div className="p-6 text-sm text-gray-500">
        Panel de diagnóstico disponible solo con <code>?debug=1</code>.
      </div>
    );
  }
