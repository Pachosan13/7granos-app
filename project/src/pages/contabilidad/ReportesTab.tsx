...
export const ReportesTab = () => {
  ...
  useEffect(()=>{ fetchData(); }, [desdeMes, hastaMes]);
  ...
  const totalize = (arr: Row[]) => arr.reduce((acc, r) => {
  ...
  const postMonths = async () => {
    ...
    setMsg(null);
    try {
      const [y1, m1] = desdeMes.split('-').map(Number);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-end gap-2 flex-wrap">
        ...
        <button onClick={postMonths} className="px-3 py-2 rounded bg-accent text-white" disabled={posting || loading} title="Postea journals de los meses seleccionados vía RPC">
                ...
                <tbody>
                  {arr.map((r, i) => (
                    <tr key={i} className="border-t">
                      <td className="p-2">{r.sucursal_id ? (sucursales[r.sucursal_id] ?? r.sucursal_id) : 'Consolidado'}</td>
      ...
      {rows.length === 0 && (
        <div className="text-slate-700">No hay asientos <b>posteados</b> en el rango seleccionado.</div>
      )}
    </div>
  );
};
